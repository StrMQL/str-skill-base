...
Web 端还支持账号级收藏、下载次数统计，以及由超级管理员维护的全局标签库：超级管理员在右上角用户菜单进入 **标签管理**（`/admin/tags`）维护标签；首页目录旁提供 **标签** 下拉多选筛选（多选为「或」，仅基于当前列表中已出现的标签）；Skill 所有者与协作者在详情页从标签库为 Skill 打标。详情页内联预览使用独立的 `/view` 接口，避免「浏览文件」计入下载次数。
